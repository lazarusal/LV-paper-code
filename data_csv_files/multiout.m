...
function varargout=multiout(train_par,train_sim,gp_ker,...
  ...
  switch method
      case 'mcmc' %if chosen method for finding the roughness hyperparameter is to sample using MCMC
          ...
          kernel=gp_ker;
            ...
            if sum(accepts<0.15)>0
                warning('some mcmc acceptance lower than 0.15')
            elseif sum(accepts>0.3)>0
            ...
            if psrf_val<1.2
                warning('psrf is good');
            else
                warning('psrf of MCMC is not lower than 1.2')
            end
      ...
      case 'optimize'
          ...
          %problem=createOptimProblem('fmincon','objective',@(x) -hyp_posterior(x,data{1},...
          %data{2},gp_ker,H,@hyp_prior),'x0',[1 1 1 1],'lb',[-10 -10 -10 -10],'ub', [10 10 10 10], 'nonlcon', @mycon);
  ...
  switch use %switching based on whether using nearest neighbours in function space...
    ...
    case 'function_local'
      ...
      em_hyp,b_gls,H,gp_ker,data,A_inv),'x0',[1 1 1 1],'lb',[0.1 0.1 0.1 0.1],'ub', [5 5 5 5]);
      gs=GlobalSearch('NumTrialPoints',2000);
      [x]=run(gs,problem);
